...
- 任何要求原生 Android 界面 + 已签名 APK 产出，且点名 MIUIX 组件库的请求

## 技术栈（已核实 2026-09 最新可用组合，全部为 Maven Central 最新版）
...
- 大标题折叠：`MiuixScrollBehavior()` + 内容 `Modifier.nestedScroll(scrollBehavior.nestedScroll.connection)`
...
- Coil：`io.coil.compose.SubcomposeAsyncImage(model, contentDescription, modifier, contentScale, loading = { ... }, error = { ... })`（commonMain）；**网络图必须** `coil-network-okhttp` 在 androidMain + `INTERNET` 权限
...
- 必须插件：`com.android.library`、`org.jetbrains.kotlin.multiplatform`、`org.jetbrains.compose`、`org.jetbrains.kotlin.plugin.compose`、`org.jetbrains.kotlin.plugin.serialization`
- `coil-network-okhttp` **只能放 androidMain**（commonMain 无该变体），否则解析失败
- `app/build.gradle.kts`：`com.android.application` + `androidx.activity:activity-compose:1.13.0`（提供 `enableEdgeToEdge`）
...
- 步骤：checkout → setup-java Zulu 21 → setup-gradle → （可选）`gradle wrapper --gradle-version 8.13` → decode signing key（Secrets）→ `./gradlew :app:assembleRelease` → upload-artifact → tag 时 `softprops/action-gh-release`
- 未配 Secrets 时 release 回退 AGP 自带 debug 签名，仍出可安装 APK
...
- 仓库需提交 `gradlew`/`gradle-wrapper.jar`，或在 workflow 用 `gradle wrapper` 生成（要求 runner 有 gradle 可用）

## 标准检测流程（交付前必做）

1. **静态检测**：资源完整性、Manifest（权限/exported/intent-filter）、Gradle 版本一致、依赖版本、CI Action 版本真实性、代码一致性（未使用 import / 类型匹配 / 稳定 key）
2. **构建**：`./gradlew :app:assembleRelease`
...
- 图片页 `PullToRefresh` 包裹 `LazyVerticalGrid` 时，grid 仍需挂 `nestedScroll(scrollBehavior.nestedScroll.connection)`
